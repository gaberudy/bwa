...
uint32_t bwtl_occ(const bwtl_t *bwt, uint32_t k, uint8_t c)
{
	...
	n += (bwt->cnt_table[b & 0xff] + bwt->cnt_table[b >> 8 & 0xff] + bwt->cnt_table[b >> 16 & 0xff] + bwt->cnt_table[b >> 24]) >> (c << 3) & 0xff;
	if (c == 0)
...
void bwtl_occ4(const bwtl_t *bwt, uint32_t k, uint32_t cnt[4])
{
	...
	x = bwt->cnt_table[b & 0xff] + bwt->cnt_table[b >> 8 & 0xff] + bwt->cnt_table[b >> 16 & 0xff] + bwt->cnt_table[b >> 24];
	x -= 15 - (k & 15);
	cnt[0] += x & 0xff;
	cnt[1] += x >> 8 & 0xff;
	cnt[2] += x >> 16 & 0xff;
