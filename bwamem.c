	...
	if (opt->max_mem_intv > 0)
	{
		...
		while (x < len)
		{
			if (seq[x] < 4)
			{
				...
				{ // for now, we never come to this block which is slower
...
int mem_chain_flt(const mem_opt_t *opt, int n_chn, mem_chain_t *a)
{
	...
	for (i = 1; i < n_chn; ++i)
	{
		...
		for (k = 0; k < chains.n; ++k)
		{
			...
			if (e_min > b_max && (!a[j].is_alt || a[i].is_alt))
			{ // have overlap; don't consider ovlp where the kept chain is ALT while the current chain is primary
...
int mem_sort_dedup_patch(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, uint8_t *query, int n, mem_alnreg_t *a)
{
	...
	for (i = 1; i < n; ++i)
	{
		...
		for (j = i - 1; j >= 0 && p->rid == a[j].rid && p->rb < a[j].re + opt->max_chain_gap; --j)
		{
			...
			if (or > opt->mask_level_redun * mr && oq > opt->mask_level_redun * mq)
			...
			else if (q->rb < p->rb && (score = mem_patch_reg(opt, bns, pac, query, q, p, &w)) > 0)
			{ // then merge q into p
				p->n_comp += q->n_comp + 1;
...
int mem_mark_primary_se(const mem_opt_t *opt, int n, mem_alnreg_t *a, int64_t id)
{
	...
	if (n_pri >= 0 && n_pri < n)
	{
		...
		if (n_pri > 0)
		{ // mark primary for hits to the primary assembly only
			for (i = 0; i < n_pri; ++i)
...
void mem_aln2sam(const mem_opt_t *opt, const bntseq_t *bns, kstring_t *str, bseq1_t *s, int n, const mem_aln_t *list, int which, const mem_aln_t *m_)
{
	...
	// print SEQ and QUAL
	if (p->flag & 0x100)
	{ // for secondary alignments, don't write SEQ and QUAL
		kputsn("*\t*", 3, str);
	}
	else if (!p->is_rev)
	{ // the forward strand
		...
		{ // have cigar && not the primary alignment && not softclip all
	...
	{ // not multi-hit
		for (i = 0; i < n; ++i)
			if (i != which && !(list[i].flag & 0x100))
				break;
		if (i < n)
		{ // there are other primary hits; output them
			kputsn("\tSA:Z:", 6, str);
			for (i = 0; i < n; ++i)
			{
...
void mem_reorder_primary5(int T, mem_alnreg_v *a)
{
	...
	mem_alnreg_t t;
	for (k = 0; k < a->n; ++k)
		if (a->a[k].secondary < 0 && !a->a[k].is_alt && a->a[k].score >= T)
			++n_pri;
	if (n_pri <= 1)
	...
	for (k = 0; k < a->n; ++k)
	{
		...
		if (p->secondary >= 0 || p->is_alt || p->score < T)
			continue;
		if (p->qb < left_st)
			left_st = p->qb, left_k = k;
	}
	...
	for (k = 1; k < a->n; ++k)
	{ // update secondary and secondary_all
		mem_alnreg_t *p = &a->a[k];
		if (p->secondary == 0)
			p->secondary = left_k;
...
static void worker2(void *data, int i, int tid)
{
	...
	if (!(w->opt->flag & MEM_F_PE))
	{
		...
		if (w->opt->flag & MEM_F_PRIMARY5)
			mem_reorder_primary5(w->opt->T, &w->regs[i]);
