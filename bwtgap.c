...
static inline void gap_pop(gap_stack_t *stack, gap_entry_t *e)
{
	...
	*e = q->stack[q->n_entries - 1];
	--(q->n_entries);
	--(stack->n_entries);
	if (q->n_entries == 0 && stack->n_entries)
	{ // reset best
		int i;
...
{ // $seq is the reverse complement of the input read
	...
	while (stack->n_entries)
	{
		...
		if (i > 0)
		{
			...
			if (seed_width && ii > 0)
			{
				if (seed_width[ii - 1].bid > m_seed - 1)
					allow_diff = 0;
				else if (seed_width[ii - 1].bid == m_seed - 1 && seed_width[ii].bid == m_seed - 1 && seed_width[ii - 1].w == seed_width[ii].w)
					allow_M = 0;
			}
		}
		// indels
		...
		if (allow_diff && i >= opt->indel_end_skip + tmp && len - i >= opt->indel_end_skip + tmp)
		{
			...
			{ // gap open
				...
				{ // gap open is allowed
					...
					for (j = 0; j != 4; ++j)
					{
						...
						l = bwt->L2[j] + cnt_l[j];
						if (k <= l)
							gap_push(stack, i + 1, k, l, e.n_mm, e.n_gapo + 1, e.n_gape, e.n_ins, e.n_del + 1, STATE_D, 1, opt);
					}
				}
			}
			else if (e.state == STATE_I)
			{								  // extention of an insertion
			...
			else if (e.state == STATE_D)
			{ // extention of a deletion
				if (e.n_gape < opt->max_gape)
				{ // gap extention is allowed
					if (e.n_gape + e.n_gapo < max_diff || occ < opt->max_del_occ)
					{
						for (j = 0; j != 4; ++j)
						{
							...
							l = bwt->L2[j] + cnt_l[j];
							if (k <= l)
								gap_push(stack, i + 1, k, l, e.n_mm, e.n_gapo, e.n_gape + 1, e.n_ins, e.n_del + 1, STATE_D, 1, opt);
						}
					}
				}
			}
