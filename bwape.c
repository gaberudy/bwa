...
#define __pairing_aux(u, v)                                                                                                                             \
	...
	{                                                                                                                                                   \
		...
		if ((u).x != (uint64_t)-1 && (v).x > (u).x && l >= max_len && ((ii->high && l <= ii->high_bayesian) || (ii->high == 0 && l <= opt->max_isize))) \
		{                                                                                                                                               \
...
#define __pairing_aux2(q, w)                                      \
	...
	if (o_score != (uint64_t)-1)
	{
		...
		if ((p[0]->pos == o_pos[0].x && p[0]->strand == (o_pos[0].y >> 1 & 1)) && (p[1]->pos == o_pos[1].x && p[1]->strand == (o_pos[1].y >> 1 & 1)))
		{ // both ends not moved
		...
		else if (p[0]->pos == o_pos[0].x && p[0]->strand == (o_pos[0].y >> 1 & 1))
		{ // [1] moved
			p[1]->seQ = 0;
			p[1]->mapQ = p[0]->mapQ;
			if (p[1]->mapQ > mapQ_p)
		...
		else if (p[1]->pos == o_pos[1].x && p[1]->strand == (o_pos[1].y >> 1 & 1))
		{ // [0] moved
			p[0]->seQ = 0;
			p[0]->mapQ = p[1]->mapQ;
			if (p[0]->mapQ > mapQ_p)
					   ...
					   const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii)
{
	...
	for (i = 0; i != n_seqs; ++i)
	{
		...
		if ((p[0]->type == BWA_TYPE_UNIQUE || p[0]->type == BWA_TYPE_REPEAT) && (p[1]->type == BWA_TYPE_UNIQUE || p[1]->type == BWA_TYPE_REPEAT))
			...
			for (j = 0; j < 2; ++j)
			{
				for (k = 0; k < d->aln[j].n; ++k)
				{
					...
					{ // then check hash table
						...
						if (ret)
						{ // not in the hash table; ret must equal 1 as we never remove elements
...
bwa_cigar_t *bwa_sw_core(bwtint_t l_pac, const ubyte_t *pacseq, int len, const ubyte_t *seq, int64_t *beg, int reglen, int *n_cigar, uint32_t *_cnt)
{
	...
	if (r.score < SW_MIN_MATCH_LEN || r.score2 == r.score || gscore != r.score)
	{ // poor hit or tandem hits or weird alignment
		free(cigar);
		free(ref_seq);
		*n_cigar = 0;
		return 0;
	...
	if (x < SW_MIN_MATCH_LEN || y < SW_MIN_MATCH_LEN)
	{ // not good enough
		free(cigar);
		free(ref_seq);
		*n_cigar = 0;
...
ubyte_t *bwa_paired_sw(const bntseq_t *bns, const ubyte_t *_pacseq, int n_seqs, bwa_seq_t *seqs[2], const pe_opt_t *popt, const isize_info_t *ii)
{
	...
	for (i = 0; i != n_seqs; ++i)
	{
		...
		p[0] = seqs[0] + i;
		p[1] = seqs[1] + i;
		if ((p[0]->mapQ >= SW_MIN_MAPQ || p[1]->mapQ >= SW_MIN_MAPQ) && (p[0]->extra_flag & SAM_FPP) == 0)
		{ // unpaired and one read has high mapQ
...
#define __set_fixed(_pref, _pmate, _beg, _cnt)                                            \
	...
	{                                                                                     \
		...
		_pref->extra_flag |= SAM_FPP;                                                     \
			...
			for (k = 0; k < 2; ++k)
			{ // p[1-k] is the reference read and p[k] is the read considered to be modified
				ubyte_t *seq;
				...
				{			  // note that popt->type == BWA_PET_STD always true; in older versions, there was a branch for color-space FF/RR reads
					if (p[1 - k]->strand == 0)
					{ // then the mate is on the reverse strand and has larger coordinate
						__set_rght_coor(beg[k], end[k], p[1 - k], p[k]);
						seq = p[k]->rseq;
					}
					else
					{ // then the mate is on forward stand and has smaller coordinate
				...
				if (cigar[k] && p[k]->type != BWA_TYPE_NO_MATCH)
				{ // re-evaluate cigar[k]
