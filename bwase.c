...
void bwa_aln2seq_core(int n_aln, const bwt_aln1_t *aln, bwa_seq_t *s, int set_main, int n_multi)
{
	...
	if (n_multi)
	{
		...
		if (s->multi)
			free(s->multi);
		if (n_occ > n_multi + 1)
		{ // if there are too many hits, generate none of them
			s->multi = 0;
			s->n_multi = 0;
			return;
		}
		...
		for (k = 0; k < n_aln; ++k)
		{
			...
			{ // Random sampling (http://code.activestate.com/recipes/272884/). In fact, we never come here.
				int j, i;
